...
@push('after-styles')
<link href="[suspicious link removed]" rel="stylesheet" />
@endpush
...
<div class="px-3 mb-4">
    <form action="{{ route('front.index') }}" method="GET">
        <div class="input-group border rounded-pill overflow-hidden shadow-sm">
            <input type="text" name="search" class="form-control border-0 ps-3" placeholder="Search kebaya..." value="{{ request('search') }}">
            <button class="btn btn-primary px-3" type="submit" id="button-addon2"><i class="bi bi-search"></i></button>
        </div>
    </form>
</div>
<div class="px-3 mb-4">
    <form action="{{ route('front.index') }}" method="GET" class="d-flex align-items-center gap-3">
        ...
        <div class="flex-grow-1 d-flex gap-3">
            <select name="size" id="size-filter" class="form-select form-select-sm rounded-pill">
            ...
            <select name="sort_price" id="price-sort" class="form-select form-select-sm rounded-pill">
